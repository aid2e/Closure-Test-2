...
from ProjectUtils.joblib_utils import JobLibRunner, JobLibMetric
...
if __name__ == "__main__":
    ...
    parser.add_argument('-r', '--runner',
                        help = "The runner to use, default is PandaIDDSRunner, other option is JoblibRunner",
                        type = str, required = False,
                        default = "PandaIDDSRunner"
                        )
    ...
    with open(os.path.join(outdir, optimInfo), "w") as f:
        ...
        f.write("Optimization is running on Runner " + runnerToUse + "\n")
        if(isGPU):
    ...
    if ("panda" in runnerToUse.lowercase()):
    ...
    # print(runner_funcs)
    if ("panda" in runnerToUse.lower()):
        runner = PanDAIDDSRunner(runner_funcs)
    else:
        runner = JobLibRunner(runner_funcs)
    ...
    while(converged > tol and last_call <= max_calls and check_imp):
        ...
        print("Waiting for trial to terminate")
        print(trial)
        time_checkpoint = time.time()
        while not trial.status.is_terminal:
            trial.runner.poll_trial_status([trial])
